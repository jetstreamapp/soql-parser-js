...
The easiest way to build the fields is to call `getComposedField()` from `soqlUtils`.
...
```typescript
import { composeQuery, queryUtils } from 'soql-parser-js';

const oppLineItemsSubquery = {
  fields: [
    queryUtils.getComposedField('Quantity'),
    queryUtils.getComposedField('ListPrice'),
    queryUtils.getComposedField({
      field: 'UnitPrice',
...
const soqlQuery = {
  fields: [
    queryUtils.getComposedField('Id'),
    queryUtils.getComposedField('Name'),
    queryUtils.getComposedField({
      fn: 'FORMAT',
      parameters: 'Amount',
      alias: 'MyFormattedAmount',
    }),
    queryUtils.getComposedField({ subquery: oppLineItemsSubquery }),
  ],
  sObject: 'Opportunity',
  where: {
    left: {
...
## Utility Functions
The following utility functions are available on the `queryUtils` object:
